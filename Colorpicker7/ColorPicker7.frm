...
Begin VB.Form frmColorPicker 
   BackColor       =   &H00C0C0C0&
   Caption         =   "Color Picker BetaVersion by Christer Holmberg"
   ClientHeight    =   4845
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7350
   Icon            =   "ColorPicker7.frx":0000
   LinkTopic       =   "Form1"
   MouseIcon       =   "ColorPicker7.frx":030A
   ...
   Begin VB.PictureBox picBigBox 
      Height          =   3870
      Left            =   225
      MouseIcon       =   "ColorPicker7.frx":045C
   ...
   Begin VB.Image imgWinter 
      Height          =   495
      Left            =   1320
      Picture         =   "ColorPicker7.frx":16621
      Stretch         =   -1  'True
      Top             =   4440
      Width           =   495
      Visible         =   0   'False
   ...
   Begin VB.Image imgMarker 
      Height          =   165
      Left            =   2535
      Picture         =   "ColorPicker7.frx":256D1
      Top             =   4560
      Width           =   165
   End
...
Dim mBlnRecentThinBoxPress As Boolean, mBlnBigBoxReady As Boolean, mBlnCursorRoutineReady As Boolean
...
Dim sAppPath As String, strPathStripC As String, strFusionPath As String, btePosEndLetter, sTest As String
Dim bteErrCtr As Byte, strAppPath As String
Dim strAPIpath As String, CSIDL_PERSONAL As Byte 'fill pidl with the specified folder item.
...
strAppPath = App.Path
If InStrRev(strAppPath, "\") <> Len(strAppPath) Then strAppPath = strAppPath & "\"  'Good windows-programming-vitue.
Call Shell(strAppPath & "ColorpickerHelp\ColorPickerHelp.html")
Exit Sub

ErrorHandler:
If Err.Number <> 53 Then MsgBox "Unknown error! " & Err.Number & Err.Description: Exit Sub 'Bails if unknown error.
'k = Err.Number Error 53 = File not found.
'MsgBox "Tries to find the helpfile in windows directory. " & Err.Number & Err.Description
bteErrCtr = bteErrCtr + 1 'COUNTING THE NUMBER OF TIMES THAT THE shell-funktion HAS FAILED.
strPathStripC = Mid(strAppPath, 4, Len(strAppPath) - 3)  'SHOULD GET EITHER THE PATH My documents\ OR Mina dokument\
Select Case bteErrCtr
    Case 1
'CHECKING THE REAL PATH FOR MYDOCUMENTS
If SHGetFolderPath(Me.hwnd, CSIDL_PERSONAL, 0&, &H0, strAPIpath) = 0 Then 'API GETS THE PATH FOR THE FOLDER THAT CONTAINS THE My dokuments FOLDER.
    btePosEndLetter = InStr(1, strAPIpath, Chr$(0), vbTextCompare) - 1 'POINTS AT THE LAST LETTER BEFORE THE SPECIAL END OF DATA SIGN.
    strAPIpath = Left(strAPIpath, btePosEndLetter) 'TRUNKATES THE USELESS SIGNS AT END OF STRING.
    If Mid(strAPIpath, btePosEndLetter, 1) <> "\" Then strAPIpath = strAPIpath + "\" 'GOOD WINDOWS PROGRAMMING MANNERS.
    'MsgBox strAPIpath
    'strAppPath=Mid(strAppPath
    strFusionPath = strAPIpath & strPathStripC & "ColorpickerHelp\ColorPickerHelp.html"
    Call Shell(strFusionPath)
    Exit Sub 'BAIL SINCE I'VE FOUND AND SHELLED THE WEBPAGE.
End If
MsgBox "Failed to find the location for My documents - sending you down to case2!": bteErrCtr = 2
    Case 2
        MsgBox "Failed in finding the helpfile ColorpickerHelp.html which should recide in the same folder as the programme ColorPicker. Would you like to browse for it manually?"
End Select
...
If Index = 2 Then ' "Brightness"
    ...
    Call picBigBox_Colorize 'Speciell design.
    'Set objAnyPictureBox = Nothing 'Maybe will save memory.
End If

If Index = 3 Then ' "R"
    'picThinBox.Visible = True
    'Call ColorCirkel("Red") 'Special design.
    'Call SampleMarkerBackground   'Saving the background behind Marker if there is any.
    ...
    Call picBigBox_Colorize 'Special design.
    
End If
If Index = 4 Then ' "G"
    'picThinBox.Visible = True
    'Call ColorCirkel("Green") 'Special design.
    'Call SampleMarkerBackground   'Saving the background behind Marker if there is any.
    'Call PaintMarker(mBteMarkerOldX, mBteMarkerOldY) 'Repainting the Marker again (if there is any).
...
'For Ctr = bteListIndex - 1 To UBound(arsPicPath) - 1 'Caounting i the array (base is 1) from the marked post to the second last post.
 '   arsPicPath(Ctr) = arsPicPath(Ctr + 1) 'Contracting the information.
'Next Ctr
'Combo1.RemoveItem (bteListIndex)  'Zerobased list.
'ReDim Preserve arsPicPath(UBound(arsPicPath) - 1) 'Schrinking the array with one cell.
...
'If blnNotFirstTimeMarker = True Then Call EraseMarker 'Deleting Marker since a new backcolor (11x11) needs to be sampled wjen this procedure is finished.

'Set objAnyPictureBox = picBigBox 'Repainting the canvas.
'*****     ********     **********     **********
...
If mBlnCursorRoutineReady = False Then MsgBox "mBlnCursorRoutineReady = False i BigBox MouseDown! Risk of artefacts!": Exit Sub
mBlnCursorRoutineReady = False 'Setting the flag to state that I'm in a new cursor round.
...
'mBlnCursorRoutineReady = True 'Trying to get rid of the artefacts that arise at fast double clicks.
...
If mBlnCursorRoutineReady = True Then Exit Sub 'Bailing if MouseUp preceeds a mousedown routine.
...
'If R > G Then lSuperior = R Else lSuperior = G 'It would be possible to make a routine half this size by an superior, but the code would be harder to understand.
...
'MsgBox "I am in the routine picThinBox_KeyDown!"
'Y = imgArrows.Top - 28
...
If Index = 1 Then
    ...
    picThinBox.Refresh
    'Set objAnyPictureBox = Nothing 'Maybe saves memory.
    End If
...
'MsgBox "Query Unload!"
For Ctr = 0 To 9 'Determin which optRadioknapp that was chosen the last session.
...
'Writing
Write #bteFileHandle, lblNewColor.BackColor 'Becomes lblOld color the next time the program starts.
